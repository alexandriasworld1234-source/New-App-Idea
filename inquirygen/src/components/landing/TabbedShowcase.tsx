...
import { X, Check, Clock, Puzzle, Target, MessageSquare, Cpu, Download, Presentation, ClipboardList, BookOpen, BarChart3, GraduationCap, Image, Brain, Layers, Users, Lightbulb } from "lucide-react";
...
const deliverables = [
  ...
  { icon: BarChart3, title: "Standards Alignment", format: null, description: "Automatically mapped to NGSS, state standards, IB, UK National Curriculum, or Australian Curriculum." },
...
const principles = [
  ...
  { icon: Layers, label: "NGSS-Aligned" },
    ...
    <div ref={ref} className="grid gap-8 lg:grid-cols-[1fr_auto_1fr]">
      ...
      <div className="space-y-6">
        <div className="liquid-glass-subtle p-6">
          <h4 className="font-display text-lg text-red-400">The Old Way</h4>
          <ul className="mt-4 space-y-3">
            {oldWay.map((item, i) => (
              <motion.li
                ...
                className="flex items-start gap-2.5 text-sm text-red-300/80"
              >
        ...
        <div className="liquid-glass-subtle p-6">
          <h4 className="font-display text-lg text-emerald-400">The InquiryGen Way</h4>
          <ul className="mt-4 space-y-3">
            {newWay.map((item, i) => (
              <motion.li
                key={item}
                ...
                className="flex items-start gap-2.5 text-sm text-emerald-300/80"
              >
        ...
        {obstacles.map((item, i) => (
          ...
          >
            <TiltCard className="liquid-glass-subtle p-5 text-center" maxTilt={6}>
              ...
              <h4 className="font-display mt-3 text-sm text-white/70">{item.title}</h4>
              <div className="mt-1">
                <span className="font-display bg-gradient-to-r from-[#10b981] to-[#34d399] bg-clip-text text-2xl font-bold text-transparent">
                  {item.stat}
                </span>
                <span className="ml-1 text-xs text-white/40">{item.statLabel}</span>
              </div>
            </TiltCard>
          </motion.div>
        ))}
      ...
      <div className="relative grid gap-8 md:grid-cols-3">
        ...
        {steps.map((step, i) => (
          ...
          >
            ...
            <h4 className="font-display mt-3 text-lg text-white">{step.title}</h4>
  ...
  return (
    <section id="features" className="relative min-h-screen py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        {/* Tab bar */}
        <div className="mx-auto mb-12 flex w-fit gap-1 rounded-full border border-white/10 bg-white/[0.03] p-1.5">
          {tabs.map((tab) => (
            <button
              key={tab.key}
